...
This runbook defines the operational procedures for monitoring, troubleshooting, and maintaining workloads running on the Kubernetes platform (Amazon EKS).

It focuses on day-2 operations, where the platform is already provisioned and managed through GitOps (ArgoCD), and emphasizes safe, observable, and controlled system behavior.

The goal is to demonstrate production-grade operational readiness through:

  * Monitoring-driven incident detection
  * Structured incident response workflows
  * Root cause analysis based on metrics and system signals
  * Safe mitigation and rollback strategies

This document assumes that:

  * Infrastructure provisioning and bootstrap are handled separately
  * Platform components are managed via GitOps
  * Operational access is governed by Kubernetes RBAC and scoped permissions

The runbook prioritizes clarity, repeatability, and real-world operational practices over complexity, ensuring that incidents can be handled efficiently without compromising system stability.
